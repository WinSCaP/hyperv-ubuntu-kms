...
# Docker with a KMS server
if [ "$INSTALL_DOCKER" == true ]; then
  echo "Provisioning phase 2 - Docker with KMS server"
  export DEBIAN_FRONTEND=noninteractive
  ...
  echo "Provisioning phase 2 - skipping Docker with KMS server"
fi
...
if [ "$INSTALL_DOCKER" == true ]; then
  echo "Phase 3 - firewalld - adding docker KMS server rules"
  ufw allow 1688:1688/tcp
  echo Port 1688 allowed
  docker run -d -p 1688:1688 --restart=always --name vlmcsd mikolatero/vlmcsd
  echo Docker KMS server created
fi
